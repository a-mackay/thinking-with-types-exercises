{-# LANGUAGE AllowAmbiguousTypes #-}
{-# LANGUAGE ConstraintKinds #-}
{-# LANGUAGE DataKinds #-}
{-# LANGUAGE FlexibleContexts #-}
{-# LANGUAGE FlexibleInstances #-}
{-# LANGUAGE GADTs #-}
{-# LANGUAGE PolyKinds #-}
{-# LANGUAGE RankNTypes #-}
{-# LANGUAGE ScopedTypeVariables #-}
{-# LANGUAGE TypeApplications #-}
{-# LANGUAGE TypeFamilies #-}
{-# LANGUAGE TypeOperators #-}
{-# LANGUAGE UndecidableInstances #-}

module Main where

import Data.Functor.Identity (Identity)
import Data.Kind (Type)
import Data.Proxy
import Fcf
import GHC.TypeLits hiding (type (+))
import Unsafe.Coerce

main :: IO ()
main = do
    putStrLn "test"

data OpenSum (f :: k -> Type) (ts :: [k]) where
    UnsafeOpenSum :: Int -> f t -> OpenSum f ts

type FindElem (key :: k) (ts :: [k])
    = FromMaybe Stuck =<< FindIndex (TyEq key) ts

-- Member is a constraint.
type Member t ts = KnownNat (Eval (FindElem t ts))

-- Need explicit forall to bring t and ts into scope for the type
-- application to Proxy.
findElem :: forall t ts. Member t ts => Int
findElem = fromIntegral . natVal $ Proxy @(Eval (FindElem t ts))

inj :: forall f t ts. Member t ts => f t -> OpenSum f ts
inj = UnsafeOpenSum (findElem @t @ts)

prj :: forall f t ts. Member t ts => OpenSum f ts -> Maybe (f t)
prj (UnsafeOpenSum i f) =
    if i == findElem @t @ts
        then Just $ unsafeCoerce f
        else Nothing

-- using inj:
-- :set -XFlexibleContexts
-- *Main> let y = inj @Identity @String $ pure "hello"
-- *Main> :t y
-- y :: KnownNat
--        (Eval (FromMaybe Stuck (Eval (FindIndex (TyEq String) ts)))) =>
--      OpenSum Identity ts

-- using prj:
-- prj @Identity @String @[String, Char] $ y
-- prj @Identity @String @'[String] $ y
-- prj @Identity @String @'[Char] $ y
-- prj @Identity @Char @'[String, Char] $ y

decompose :: OpenSum f (t ': ts) -> Either (f t) (OpenSum f ts)
decompose (UnsafeOpenSum 0 t) = Left $ unsafeCoerce t
decompose (UnsafeOpenSum n t) = Right $ UnsafeOpenSum (n - 1) t

-- weaken :: OpenSum f ts -> OpenSum f (x ': ts)
-- weaken (UnsafeOpenSum n t) = UnsafeOpenSum 